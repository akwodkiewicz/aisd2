...
namespace ASD
{
    ...
    class SutherlandHodgmanTestCase : TestCase
    {
        ...
        public override void VerifyTestCase(out Result resultCode, out string message)
        {
            ...
            {
                if(intersectedPolygon[index % intersectedPolygon.Length] != expectedIntersectedPolygon[i])
                {
            ...
            for (int i = 0, index = startIndex; i < intersectedPolygon.Length; i++, index--)
            {
                if (intersectedPolygon[(index + intersectedPolygon.Length) % intersectedPolygon.Length] != expectedIntersectedPolygon[i])
                {
                    isOrderCorrect = false;
                    break;
                }
